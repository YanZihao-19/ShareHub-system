...
public interface ItemMapper {
    ...
    @Insert("insert into item (suit,image,delivery_style,address,usage_level,original_price,sell_price,owneruid,status,trade_mode,tag,item_desc,item_title,create_time, update_time) " +
            "values (#{suit},#{image},#{deliveryStyle},#{address},#{usageLevel},#{originalPrice},#{sellPrice},#{ownerUid},#{status},#{tradeMode},#{tag},#{itemDesc},#{itemTitle},#{createTime},#{updateTime})")
    ...
    //不同交易模式的物品列表随机获取6条数据
    List<Item> getItemList(Integer mode, Integer tag);
