<template>
	...
	<view class="">
		...
		<!-- <tabBar></tabBar> -->
		<!-- 导航条 -->
		<bar @click="tabSelect" :TabCur="TabCur" :dataList="tablist"></bar>
		<!-- 导航条 -->
			...
			<view class='contianer shadow-warp bg-white' v-for="(item,index) in 10" :key="index">
				...
				<view class='container-grid'>
					<view @tap='previewImg' :data-index='index'
						...
						:style="imgArr.length == 4 ? 'width:32%;margin-right: 15rpx;' : '' "
						v-for="(item,index) in imgArr" :key="item">
				...
				<view class='container-evaluate bg-gray'>
					<view class='container-evaluate-top'>
						<view class='container-evaluate-top_1'>
							<image src='../../static/img/hot_evaluate1.png'></image>
						</view>
						<view>300 <text class='text-sm'>赞</text></view>
					</view>
					<view class='container-evaluate-desc'><text
							class='text-sm text-weight '>啊手动阀:按实际发生,分解凯撒就打发士大夫萨芬就是垃圾地方就是咖啡碱撒扩大啊手动阀按实际发生分解凯撒就打发士大夫萨芬就是垃圾地方就是咖啡碱撒扩大</text>
					</view>
				</view>
