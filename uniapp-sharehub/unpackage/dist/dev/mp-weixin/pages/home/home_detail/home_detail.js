  ...
  methods: {
    ...
    imageStyle: function imageStyle(item) {
      ...
      return function (item) {
        var v = uni.upx2px(750) - _this4.leftGap - _this4.rightGap - _this4.columnGap;
        var w = v / 2;
        var rate = w / item.w;
        var h = rate * item.h;
        return {
    ...
    getData: function getData(id) {
      ...
      return new Promise(function (resolve, reject) {
        ...
        uni.request({
          url: 'http://localhost:8080/items/recommendItems?tag=' + _this5.tag + '&mode=' + _this5.mode + '&id=' + id,
          method: 'POST',
          header: {
    ...
    getUser: function getUser(openId) {
      ...
      return new Promise(function (resolve, reject) {
        //获取token
        _this9.token = uni.getStorageSync('token');
        uni.request({
    ...
    getItemData: function getItemData(id) {
      ...
      return new Promise(function (resolve, reject) {
        //获取token
        _this10.token = uni.getStorageSync('token');
        uni.request({
          ...
          success: function success(res) {
            ...
            if (res.data && res.data.code === 1) {
              //赋值给前端的 item 数据
              _this10.item = res.data.data.item;
              console.log("物品种类：", _this10.item.tag);
              _this10.itemImages = res.data.data.itemImages;
              // console.log('this.item的address数据：', this.item.address)
              //修改日期格式
              _this10.item.createTime = _this10.item.createTime.replace('T', ' ');
              // console.log('this.item.time的数据：', this.item.createTime)
              _this10.getUser(_this10.item.ownerUid);
